...
def get_covers(MARKET: str = 'SNP',  # currently valid only for SNP!
    ...
    strkDelta = np.where(df_ch3.right == 'C',
                         df_ch3.undPrice * df_ch3.und_iv * (df_ch3.dte / 365)
                         .apply(math.sqrt), -df_ch3.undPrice *
                         df_ch3.und_iv * (df_ch3.dte / 365)
                         .apply(math.sqrt))
    ...
    optcon_raw = [Option(s, e, k, r, 'SMART')
                  for s, e, k, r
                  in zip(df_ch5.symbol, df_ch5.expiry,
                         df_ch5.strike, df_ch5.right)]
    ...
    df_ch6 = df_ch5.set_index(cols).join(
    ...
    missing_und_symbols = [
        s for s in uncov if s not in list(df_covers.symbol.unique())]
