...
def get_dfrq(MARKET: str) -> pd.DataFrame:
    ...
    dfrq = df_unds.assign(lot=df_unds.symbol.map(lotmap), gross=s_gross).\
        sort_values('gross', ascending=False)[['symbol', 'undPrice', 'lot', 'gross']].\
        reset_index(drop=True)

    # * BUILD dfrq
    # .integrate grosspos into dfrq
    dfrq = dfrq.assign(grosspos=dfrq.symbol.map(df_gp).
                       apply(abs)).sort_values('grosspos', ascending=False)
    ...
    m_undefended = df_pf.symbol.isin(df_pf[df_pf.secType == 'STK'].symbol) & \
    ...
    m_uncovered = df_pf.symbol.isin(df_pf[df_pf.secType == 'STK'].symbol) & \
        (~df_pf.symbol.isin(partials)) & \
        (df_pf.secType == 'OPT') & \
        (df_pf.position > 0)
