...
def get_nakeds(MARKET: str,
    ...
    if (SYMBOL == '') & (EARLIEST == False) & (SAVE == True): # default for save all nakeds
    ...
    # . remove opt columns
    cols1 = ['conId', 'symbol', 'strike', 'expiry', 'secType', 'right', 'contract', 'lot']
    ...
    if EARLIEST: # keep only the earliest expiring dte for each symbol
        df_opts = df_opts[df_opts.dte == \
                            df_opts.groupby('symbol').dte.transform(min)]

    else:# remove dtes between MINDTE and MAXDTE
        df_opts = df_opts[df_opts.dte.between(ibp.MINDTE, 
                                                    ibp.MAXDTE, 
                                                    inclusive=True)]
    ...
    df_opts = (df_opts.set_index("symbol")
    ...
    df_opts = df_opts.set_index(frcols).join(df_fr1[frcols+['fallrise']].set_index(frcols)).reset_index()
    ...
    call_mask = (df_opts.right == 'C') & (df_opts.und_sd > ibp.CALLSTDMULT ) & (df_opts.strike > df_opts.undPrice)
    put_mask = (df_opts.right == 'P') & (df_opts.und_sd > ibp.PUTSTDMULT ) & (df_opts.strike < df_opts.undPrice)
    df_opts = df_opts[call_mask|put_mask].reset_index(drop=True)

    # . remove opts above MAXOPTQTY_SYM

    df_opts= df_opts.assign(value=np.where(df_opts.right == 'C', 
                                    -1*df_opts.strike, 
                                    df_opts.strike))\
                .sort_values(['symbol', 'dte', 'value'])

    df_nakeds = df_opts[df_opts.groupby(['symbol', 'right', 'dte'])\
                            .cumcount() < ibp.MAXOPTQTY_SYM]\
                        .drop('value', 1)\
    ...
    df_nakeds = df_nakeds.assign(intrinsic=np.where(
                    df_nakeds.right == "C",
    ...
    # . compute time values
    df_nakeds = df_nakeds.assign(timevalue=df_nakeds.price - \
                                df_nakeds.intrinsic)
    ...
    df_nakeds = df_nakeds[df_nakeds.rom > 0]\
    ...
    df_nakeds["expPrice"] = (df_nakeds.expRom
        * np.maximum(ibp.MINOPTSELLPRICE, df_nakeds.price)
        / df_nakeds.rom).apply(lambda x: get_prec(x, ibp.PREC))
    ...
    if SAVE:
        ...
        writer = pd.ExcelWriter(DATAPATH.joinpath(XL_FILE), 
                                engine="xlsxwriter")
...
if __name__ == "__main__":
    ...
    SAVE = input('Do you want to output result to file? ')
    ...
    print(y.drop(['conId', 'contract', 'secType', 'comm', 
                    'lot', 'close', 'last', 'intrinsic'], 1))
