...
from support import Timer, Vars, calcsdmult_df, get_dte, get_prec, get_prob, quick_pf
...
def get_orphans(
    ...
    df_ch = get_chains(MARKET=MARKET, und_cts=orph_cts, SAVE=False, RUN_ON_PAPER=False)
    df_u = get_unds(MARKET=MARKET, und_cts=orph_cts, SAVE=False, RUN_ON_PAPER=False)
    ...
    df_pf1 = df_pf[m][
        ["symbol", "position", "avgCost", "strike", "right", "expiry"]
    ].rename(columns={"strike": "old_strk", "expiry": "old_exp"})

    df_ch3 = (
        df_ch2.join(df_pf1.set_index("symbol"))
        .reset_index()
        .rename(columns={"position": "qty"})
    )

    df_ch3["old_dte"] = df_ch3.old_exp.apply(get_dte)

    df_ch3["old_sdMult"] = calcsdmult_df(
        df_ch3.old_strk, df_ch3.assign(iv=df_ch3.und_iv, dte=df_ch3.old_dte)
    )
    ...
    optcon_raw = [
        ...
        for s, e, k, r in zip(df_ch4.symbol, df_ch4.expiry, df_ch4.strike, df_ch4.right)
    ]
    ...
    df_ch7["expPrice"] = (df_ch7.price + 3 * ibp.PREC).apply(
    ...
    df_ch8 = df_ch8.assign(
        pnl=df_ch8.revenue - df_ch8.avgCost * df_ch8.lot * df_ch8.qty
    )
    ...
    df_ch8 = df_ch8[
        df_ch8.pnl == df_ch8.groupby("symbol").pnl.transform(max)
    ].reset_index(drop=True)

    # ... report missing underlying symbols
    missing_und_symbols = [
        s for s in orph if s not in list(df_raw_orphs.symbol.unique())
    ]
    ...
    if SAVEXL:
        writer = pd.ExcelWriter(DATAPATH.joinpath("propose_orphans.xlsx"))
        df_orphans.to_excel(
            writer,
            sheet_name="Orphans",
            float_format="%.2f",
